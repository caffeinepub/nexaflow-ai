...
function isUserCancellation(msg?: string): boolean {
  ...
  return USER_CANCEL_MESSAGES.some((m) => lower.includes(m.toLowerCase()));
}
...
export function InternetIdentityProvider({
  ...
  const handleLoginError = useCallback(
    (maybeError?: string) => {
      if (!maybeError || isUserCancellation(maybeError)) {
        setStatus("idle");
        setError(undefined);
        return;
      }
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: run once on mount using ref
  useEffect(() => {
